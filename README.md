...
The landing page (`/`) is now the **visual Trimmer** — paste a link and the video
loads there to hand-trim (see below). The **auto-clip pipeline + gallery** live at
`/clips` (linked as "Auto-clip & gallery" in the header). While a clip is running,
a **Cancel** button appears in the job panel (both pages) that stops it and its
ffmpeg/whisper/yt-dlp children.
...
#### Visual trimmer (the landing page)
The Trimmer is the front page (`/`, also `/editor`): **paste a video URL or path
and click Load** — it downloads into `media/` (progress shown) and the video
appears — or pick one from the dropdown. It loads on a **timeline with a
thumbnail filmstrip and an audio waveform** (both generated by ffmpeg and cached
per source). Drag the **in/out handles** (or use *Set In* `[` / *Set Out* `]`,
or type times), **loop-preview** the selection, then **Cut selection** exports
that exact window as a 9:16 clip — optionally with **face-track reframe** or
**facecam split**. Loudness is normalized; captions aren't burned on a hand-trim
(they come from the transcript in the auto-pipeline). Keys: space = play/pause,
`i`/`o` = set in/out. It's all local — the source streams from your disk with
HTTP Range, and the clip lands in the gallery.
